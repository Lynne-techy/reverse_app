...
(async () => {
  ...
  await build("인증 콜백", async () => callback(DW, DH, true));
  await build("홈", async () => { const s = desktop("홈", "홈"); await pageHome(s.main, true); return s.frame; });
  await build("필사 ① 범위", async () => { const s = desktop("필사 ① 범위", "필사"); const col = dcol(s); pHead(col, true); pProg(col, 1, true); await pStepRange(col); await pActs(col, 1); return s.frame; });
  await build("필사 ② 언어", async () => { const s = desktop("필사 ② 언어", "필사"); const col = dcol(s); pHead(col, true); pProg(col, 2, true); await pStepLang(col); await pActs(col, 2); return s.frame; });
  await build("필사 ③ 사진", async () => { const s = desktop("필사 ③ 사진", "필사"); const col = dcol(s); pHead(col, true); pProg(col, 3, true); await pStepPhoto(col); await pActs(col, 3); return s.frame; });
  await build("필사 ④ Key Verse", async () => { const s = desktop("필사 ④ Key Verse", "필사"); const col = dcol(s); pHead(col, true); pProg(col, 4, true); await pStepKey(col); await pActs(col, 4); return s.frame; });
  await build("필사 ⑤ QT", async () => { const s = desktop("필사 ⑤ QT", "필사"); const col = dcol(s); pHead(col, true); pProg(col, 5, true); await pStepQt(col); await pActs(col, 5); return s.frame; });
  await build("필사 저장 완료", async () => { const s = desktop("필사 저장 완료", "필사"); const col = dcol(s); await pSuccess(s.frame, col, true); return s.frame; });
  await build("필사 저장 실패", async () => { const s = desktop("필사 저장 실패", "필사"); const col = dcol(s); await pError(col, true); return s.frame; });
