...
(async () => {
  const CPAGE = "01 Components", SPAGE = "03 Mobile";
  ...
  const board = AL("VERTICAL", { name: "Screen Flow — Mobile", itemSpacing: 16, paddingTop: 56, paddingBottom: 56, paddingLeft: 56, paddingRight: 56 });
  setFill(board, "fill"); sp.appendChild(board); board.x = 0; board.y = 0;
  board.appendChild(T("모바일 (390 × 844)", "Bold", 22, "ink"));
  ...
  await build("에러", async () => await errorScreen(MW, MH, false));
  ...
  await figma.setCurrentPageAsync(sp);
  figma.notify(errors.length ? ("모바일 v4 · 에러 " + errors.length + "건 — 보드 상단 확인") : ("모바일 v4 완료 — " + cells.children.length + "화면"));
  console.log("mobile v4 — " + cells.children.length + (errors.length ? ("\n" + errors.join("\n")) : ""));
})();
