  ...
  useEffect(() => {
    ...
    const loadProfileData = async () => {
      try {
        ...
        const [profileData, progressData, statisticsData, sessionResult] = await Promise.all([
          getMyProfile(),
          getUserProgress(),
          getMyStatistics(),
        ...
        const name = profileData.displayName || metadata.full_name || metadata.name || "사용자";
    ...
    return (
      <main className="app-page">
        ...
        <section className="mt-8 rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          <p className="m-0 text-sm text-slate-500">프로필 정보를 불러오는 중...</p>
        </section>
      </main>
    );
  }
  ...
  return (
    <main className="app-page">
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="min-w-0">
            ...
            <p className="mb-0 mt-2 text-xs text-white/75">
              {progress.completedBooks}권 완필 · {statistics.currentStreak}일 연속
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-5">
          <strong className="block text-2xl text-slate-900">{progressRate.toFixed(1)}%</strong>

          <small className="mt-2 block text-sm text-slate-500">진척률</small>
        </article>

        <article className="rounded-2xl border border-slate-200 bg-white p-5">
          <strong className="block text-2xl text-slate-900">{progress.completedBooks}</strong>
      ...
      <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-6">
        <span className="text-sm font-medium text-slate-500">전체 성경 필사 진척률</span>
        ...
        <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-3">
          {achievements.map((achievement) => (
            ...
            >
              ...
              <h3 className="mb-0 mt-4 text-base font-bold text-slate-900">{achievement.title}</h3>
